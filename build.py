...
def get_header(title, active_page):
    ...
    for p in ['index', 'about', 'snacks', 'candies', 'toys', 'others', 'contact']:
        header = header.replace(f"{{nav_{p}}}", "active" if p == active_page else "")
    return header

def render_product_grid(products_list):
...
for cat in categories:
    content = get_header(cat['title'], cat['key'])
    content += f'<div class="page-container animate-in"><h2>{cat["title"]}區</h2></div>'
    content += render_product_grid([p for p in products if p['category'] == cat['filter']])
    content += footerTemplate
    with open(cat['file'], 'w', encoding='utf-8') as f:
        f.write(content)

# Write About Page
aboutContent = get_header('公司簡介', 'about')
aboutContent += """
...
with open('about.html', 'w', encoding='utf-8') as f:
    f.write(aboutContent)

# Write Contact Page
contactContent = get_header('聯絡我們', 'contact')
contactContent += """
<div class="page-container animate-in">
    <h2>聯絡我們</h2>
...
with open('contact.html', 'w', encoding='utf-8') as f:
    f.write(contactContent)

# Write Product Pages
for p in products:
    pContent = get_header(p['name'], '')
    pContent += f"""
