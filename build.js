...
function getHeader(title, activePage) {
    ...
    const pages = ['index', 'about', 'snacks', 'candies', 'toys', 'others', 'contact'];
    pages.forEach(p => {
        header = header.replace(\`{{nav_\${p}}}\`, p === activePage ? 'active' : '');
    });
    return header;
}

function renderProductGrid(productsList) {
    let html = '<section class="products-grid animate-in">';
...
categories.forEach(cat => {
    let content = getHeader(cat.title, cat.key);
    content += \`<div class="page-container animate-in"><h2>\${cat.title}區</h2></div>\`;
    content += renderProductGrid(products.filter(p => p.category === cat.filter));
    content += footerTemplate;
    fs.writeFileSync(path.join(__dirname, cat.file), content);
});

// Write About Page
let aboutContent = getHeader('公司簡介', 'about');
aboutContent += \`
...
// Write Contact Page
let contactContent = getHeader('聯絡我們', 'contact');
contactContent += \`
<div class="page-container animate-in">
    <h2>聯絡我們</h2>
...
products.forEach(p => {
    ...
    fs.writeFileSync(path.join(__dirname, \`product\${p.id}.html\`), pContent);
});
